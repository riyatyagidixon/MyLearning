...
def test_automation_logo(log_filename):    
    ...
    ascii_company_name_centered = "\n".join([line.ljust(terminal_width) for line in ascii_company_name.split("\n")])

    # Make the second line full width
    ascii_banner_name_full = "\n".join([line.center(terminal_width) for line in ascii_banner_name.split("\n")])
    ...
    with open(log_filename, 'a') as log_file:
        try:
            ...
            if ssid in scan_result:
                ...
                log_file.write(f"SSID '{ssid}' not found in the scan result after refresh." + '\n')
...
def cal_time_duration(start_time, log_filename, task_name):
    with open(log_filename, 'a') as log_file:
        ...
        print(Fore.CYAN + f"Your {task_name} Took : {int(hours)} hrs, {int(minutes)} mins, {int(seconds)} secs")
        log_file.write(f"Your {task_name} Took : {int(hours)} hrs, {int(minutes)} mins, {int(seconds)} secs" + '\n')
...
if __name__ == "__main__":
    ...
    target_ssid = input(Fore.YELLOW + "Enter your SSID like {Riya's Galaxy Z Flip4}: " + Style.RESET_ALL) or "Riya's Galaxy Z Flip4"
    password = input(Fore.YELLOW + "Enter your SSID Password: " + Style.RESET_ALL) or "nhibtati"
