...
from Dixon_Graph_Generator_Exp import create_blank_excel, populate_data, add_chart, display_chart
...
def extract_iperf_data_from_file(file_name):
    ...
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    print(f"{Fore.CYAN}iperf3 executed commands count:{Style.RESET_ALL} [{len(commands)}]")
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    for cmd in commands:
        if counting <= 10:
            print(f"{cmd}")
            ...
            if counting == 10:
               print(f"<===================={Fore.MAGENTA}Bit-Rate-Count{Style.RESET_ALL}[{counting}]==================================>") 
        elif counting > 10 and counting <= 20:
            print(f"{cmd}")
            pktlen_val.append(variable_regexp.search(cmd)[0])
            if counting == 20:
                print(f"<===================={Fore.MAGENTA}Packet-Len-Count{Style.RESET_ALL}[{counting}]==================================>") 
        elif counting > 20 and counting <= 30:
            print(f"{cmd}")
            time_val.append(variable_regexp.search(cmd)[0])
            if counting == 30:
                print(f"<===================={Fore.MAGENTA}Time-Duration-Count{Style.RESET_ALL}[{counting}]==================================>") 
        counting = counting + 1

  
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    print(f"{Fore.CYAN}iperf3 receiver statistics count:{Style.RESET_ALL} [{len(receiver_stats)}]")
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    counting = 1
    ...
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    print(f"{Fore.CYAN}iperf3 traffic loss percentage count:{Style.RESET_ALL} [{len(receiver_stats)}]")
    print(f"{Fore.BLUE}--------------------------------------------------------------------------------------------------------------{Style.RESET_ALL}")
    counting = 1
    for stats in receiver_stats:
        if counting <= 10:
...
def get_excel_sheet_info(file_name_1, file_name_2):
    ...
    return excel_sheet_name, first_vendor_name, first_vendor_type, sec_vendor_name, sec_vendor_type
...
def get_file_name_and_path(file_name):
    ...
    if filename.suffix == ".txt":
        ...
        if filepath:
            ...
            print(f"{Fore.LIGHTBLUE_EX}{filename}{Fore.RED} file not found.{Style.RESET_ALL}")
            sys.exit(0)
    else:
...
if __name__ == '__main__':
    ...
    # Get the log files name from user
    file_name = input(Fore.YELLOW + "Enter your first filename : " + Style.RESET_ALL) or "log_2024-10-23_13-11-28_Airtel_Zerotouch_5G_ZX_6_PP.txt"
    first_file_name, first_file_path = get_file_name_and_path(file_name)
    
    file_name = input(Fore.YELLOW + "Enter your second filename : " + Style.RESET_ALL) or "log_2024-10-23_11-42-04_Airtel_Zerotouch_5G_DXB_6_PP.txt"    
    sec_file_name, sec_file_path = get_file_name_and_path(file_name)
    
    # Get excel-sheet name based on files name
    excel_sheet_name, first_vendor_name, first_vendor_type, sec_vendor_name, sec_vendor_type = get_excel_sheet_info(first_file_name, sec_file_name)
    
    # Create the excel-sheet
    excel_file_path = Path.cwd() / 'Logs' /f"{excel_sheet_name}.xlsx"
    create_blank_excel(excel_file_path)
    print(f"{Fore.CYAN}Excel file created at: {Fore.GREEN}{excel_file_path}{Style.RESET_ALL}")

    # Extract the data from provided log files
    bitrate_val_1, bitrate_loss_1, pktlen_val_1, pktlen_loss_1, time_val_1, time_loss_1 = extract_iperf_data_from_file(first_file_path)
    bitrate_val_2, bitrate_loss_2, pktlen_val_2, pktlen_loss_2, time_val_2, time_loss_2 = extract_iperf_data_from_file(sec_file_path)
